
const bookingsLink = document.querySelector('.bookings-link');
const bookings = document.querySelector('.bookings');
const closeBtn = document.querySelector('.close-btn');

bookingsLink.addEventListener('click', (event) => {
  event.preventDefault();
  bookings.classList.remove('hidden');
});

closeBtn.addEventListener('click', () => {
  bookings.classList.add('hidden');
});

// Add event listener for change status buttons
const changeStatusBtns = document.querySelectorAll('.change-status-btn');
changeStatusBtns.forEach(btn => {
  btn.addEventListener('click', () => {
    const table = btn.parentElement.dataset.table;
    alert(`Changing status for ${table}`);
    // Add your logic to change the status here
  });
});
